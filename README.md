...
In mathematic expression, for each filter index $idx$, let the model after 1st layer CNN be $\mathcal{F}: ℝ^{512×256} → ℝ^3$, we have used $𝑥 = [𝑎_{𝑖𝑗}] ∈ ℝ^{512×256}$, <img src="https://render.githubusercontent.com/render/math?math=a_{ij}= \begin{cases} 1,& \text{if } j = idx\\ 0,& \text{otherwise} \end{cases}">
